...
export const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-24 relative overflow-hidden border-t border-rose-900/40">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <ScrollReveal>
          <div className="flex items-center gap-3 mb-4">
            <span className="font-mono text-rose-400 text-xs tracking-widest uppercase font-semibold">Technical Stack</span>
          </div>

          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            <div>
              <h2 className="text-3xl sm:text-5xl font-bold tracking-tight text-white">
                Skills & <span className="text-rose-400 font-mono">Micro-Matrix</span>
              </h2>
              ...
              {skillCategories.map((cat, idx) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg font-mono text-xs transition-all ${
                    activeCategory === idx
                      ? 'bg-rose-600 text-white font-bold shadow-lg border border-rose-400/40'
        ...
        <ScrollReveal delay={150}>
          <SpotlightCard className="p-8 mb-8 border-rose-900/60">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                {React.createElement(skillCategories[activeCategory].icon, { className: "w-7 h-7 text-rose-400" })}
                <div>
              ...
              <span className="px-3 py-1 rounded-full bg-rose-950 border border-rose-800/60 text-rose-300 font-mono text-xs">
              ...
              {skillCategories[activeCategory].skills.map((skill, sIdx) => (
                <div key={sIdx} className="space-y-2">
                  ...
                  <div className="h-2.5 w-full bg-slate-950 rounded-full overflow-hidden p-0.5 border border-slate-800">
                    <div
                      className="h-full bg-gradient-to-r from-rose-600 to-rose-400 rounded-full transition-all duration-700 ease-out shadow-[0_0_10px_#e11d4880]"
        ...
        <div className="space-y-4">
          <h4 className="text-xs font-mono text-slate-400 uppercase tracking-widest mb-2 font-semibold">Expand All Skill Categories (Slide-Down View)</h4>
          {skillCategories.map((cat, idx) => (
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {cat.skills.map((s, sIdx) => (
                    <div key={sIdx} className="p-3.5 rounded-xl bg-slate-950 border border-slate-800">
                      <div className="flex justify-between font-bold text-white text-sm mb-1">
                        <span>{s.name}</span>
                        <span className="text-rose-400 font-mono text-xs">{s.level}%</span>
                      </div>
