  ...
  return (
    <section id="certifications" className="py-20 relative overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <ScrollReveal>
          <div className="mb-3">
            <span className="text-rose-300/90 text-xs font-semibold tracking-wider uppercase">Verification & Credentials</span>
          </div>
        ...
        <div className="space-y-5">
          ...
          <ScrollReveal delay={100}>
            <SpotlightCard className="p-6 border-rose-950/50 bg-rose-950/20">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
                <div className="flex items-center gap-3">
                  <div className="p-3 rounded-xl bg-rose-950/60 text-rose-300">
                    <Database className="w-5 h-5" />
                  </div>
                  <div>
                    <span className="px-2.5 py-0.5 rounded bg-rose-950/60 text-rose-200/90 text-[11px]">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xs text-rose-300 block font-mono">ID: MDBnoiow5b82a</span>
                </div>
              </div>
                ...
                {certifications.map((item, idx) => (
                  <SpotlightCard key={idx} className="p-4">
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <span className="px-2 py-0.5 rounded bg-rose-950/40 text-rose-300 text-[10px] font-medium">
                        {item.badge}
                      </span>
                    ...
                    {item.certId && (
                      <span className="text-[10px] text-rose-300 mt-2 block font-mono">
                        Cert ID: {item.certId}
                      </span>
                    )}
                  </SpotlightCard>
              ...
              <div className="space-y-4">
                {hackathons.map((h, idx) => (
                  <SpotlightCard key={idx} className="p-5">
                    ...
                    <p className="text-xs text-rose-300/80 mb-2">{h.organizer}</p>
