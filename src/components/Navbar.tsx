  ...
  return (
    <>
      ...
      <header className="fixed top-6 right-6 z-50 hidden md:block">
        <nav className="flex items-center gap-2 p-2 rounded-2xl bg-slate-950/90 backdrop-blur-md border border-slate-800 shadow-xl">
          {/* Section Nav Pill Links (Clean, No '//') */}
          {navItems.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className={`px-4 py-2 rounded-xl text-xs font-medium tracking-wide transition-all duration-300 ${
                activeSection === item.id
                  ? 'text-sky-300 font-semibold bg-slate-900 border border-sky-500/30'
          ...
          <div className="flex items-center gap-2 pl-2 ml-1 border-l border-slate-800">
            ...
            <MagneticButton onClick={onOpenResume}>
              <span className="px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold text-xs uppercase tracking-wider transition-all flex items-center gap-1.5 shadow-md">
      ...
      <div className="fixed top-4 right-4 z-50 md:hidden">
        <button
          ...
          className="p-3 rounded-xl bg-slate-950/90 backdrop-blur-md text-slate-200 border border-slate-800 shadow-lg"
        >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-16 right-4 z-50 w-64 bg-slate-950 border border-slate-800 rounded-2xl p-5 flex flex-col gap-3 text-xs uppercase tracking-wider backdrop-blur-2xl shadow-2xl text-right">
          {navItems.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              onClick={() => setMobileMenuOpen(false)}
              className="text-slate-300 hover:text-sky-400 py-2 border-b border-slate-900 w-full font-medium"
            >
              {item.label}
            </a>
          ))}
          ...
          <div className="flex items-center justify-end gap-3 pt-3 w-full">
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-sky-500 text-slate-950 font-bold text-xs uppercase"
            >
              Resume
            </button>
          </div>
