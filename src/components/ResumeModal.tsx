...
import { X, Download, FileText, ExternalLink, Mail, Phone, MapPin, CheckCircle2, Database, Eye } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/80 backdrop-blur-md">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono">Official Resume Document</p>
            </div>
          </div>
            ...
            <div className="bg-slate-800 p-1 rounded-lg flex items-center gap-1 text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-md font-medium transition-all ${
                  viewMode === 'pdf' ? 'bg-sky-600 text-white shadow' : 'text-slate-400 hover:text-white'
                }`}
              >
                PDF View
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-md font-medium transition-all ${
                  viewMode === 'interactive' ? 'bg-sky-600 text-white shadow' : 'text-slate-400 hover:text-white'
                }`}
              >
                Text View
              </button>
        ...
        {/* Modal Body - PDF Document Viewer or Interactive Text View */}
        {viewMode === 'pdf' ? (
          <div className="flex-1 w-full h-[70vh] bg-slate-950 p-2 relative">
            <object
              data={pdfUrl}
              type="application/pdf"
              className="w-full h-full rounded-xl border border-slate-800"
            >
              <div className="p-8 text-center text-slate-300 flex flex-col items-center justify-center h-full gap-4">
                ...
                >
                  Click Here to Download AKASH_SURESH_RESUME.pdf
                </a>
              </div>
            </object>
          </div>
